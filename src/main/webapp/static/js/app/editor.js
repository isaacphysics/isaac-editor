...
function docChanged(c,oldDoc,newDoc) {
	...
	history[c].push(oldDoc);
	c.setProps({doc: newDoc});
}

function undo(e) {
	e.setProps({doc: history[e].pop()});
}

function createEditor(container, document) {
	console.log("Loading doc into JSON editor:", document);
	var editor = <ContentBlock doc={document} onChange={docChanged} blockTypeTitle="Content Object"/>;
	history[editor] = [];
	React.renderComponent(editor, container);
	return editor;
}
